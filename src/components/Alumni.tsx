    ...
    try {
        useEffect(() => {
            async function getAlumni() {
                const response = await fetch("/api/alumni", { next: { revalidate: 10 } });
                    ...
                    {alumni.map((student) => (
                        <div className="flex flex-col items-center py-3 min-w-[40%] lg:min-w-[20%] lg:py-5" key={student.id} >
                            <div className="relative w-24 h-24 overflow-hidden rounded-full">
                                <Image src={student.upload.link} alt="Alumni" fill className="absolute top-0 left-0 object-cover w-full h-full rounded-full"/>
                            </div>
